...
sub remove_user_in_account{
	...
	print "########################### DELETE-USER-IN-ACCOUNT ##############################\n";
        ...
        print "$ENV{'QA_CLC_IP'} :: euare-userdel -u $user\n";
        system("ssh -o ServerAliveInterval=1 -o ServerAliveCountMax=5 -o StrictHostKeyChecking=no root\@$ENV{'QA_CLC_IP'} \"source /root/cred_depot/$account/admin/eucarc; euare-userdel -u $user \" ");
        print "\n";
